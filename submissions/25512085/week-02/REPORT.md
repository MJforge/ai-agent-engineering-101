...
### Plan-then-Execute v2 조건 (재실행 전 정의)

v1의 13번과 14번은 첫 계획 응답이 JSON 리스트가 아니어서 도구 실행 전에
종료했다. 이 형식 오류 복구 축을 분리해 관찰하기 위해 v2에서는 첫 계획의
`parse_plan` 실패 시에만 고정된 교정 프롬프트로 계획을 한 번 다시 요청한다.
교정 프롬프트는 JSON 배열만 반환할 것, 도구 호출·설명·Markdown 코드 펜스를
포함하지 말 것, 3--5개의 짧은 단계 문자열을 쓸 것을 명시하고 예시 배열도
제공한다. 두 번째 응답도 파싱에 실패하면 `X`로 종료한다.

따라서 v2의 `max_plan_retries=1`은 실행 전에 코드에 고정한 오류 복구 한도다.
재시도에 든 모델 호출·토큰은 `Meter`에 그대로 포함한다. ReAct 코드와 모델,
태스크, 공용 도구, 성공 판정 기준은 바꾸지 않는다. v2의 16--21번 6회 실행은
v1 결과와 구분해 추가 기록했으며, 아래 v1 측정표를 덮어쓰지 않는다.
...
| 하네스 | 실행 번호 | 성공 | 토큰 수 | 반복 횟수 | 개입 횟수 | 핵심 로그 |
| --- | --- | --- | --- | --- | --- | --- |
| ReAct | 16 | X | 3,638 | 2 | 0 | 로그를 읽었지만 최종 답변에 `14:00`을 포함하지 못함 |
| ReAct | 17 | O | 4,223 | 2 | 0 | `Answer: 14:00` 반환 |
| ReAct | 18 | O | 7,533 | 3 | 0 | `read_file`, `count_pattern` 뒤 `Answer: 14:00` 반환 |
| Plan-then-Execute | 19 | O | 19,571 | 9 | 0 | 첫 계획 `read_file(app.log)`를 거부한 뒤 재요청으로 JSON 계획을 받아 성공 |
| Plan-then-Execute | 20 | X | 측정 불가 | 측정 불가 | 측정 불가 | 실행 2단계에서 `TypeError: 'NoneType' object is not subscriptable` 발생 |
| Plan-then-Execute | 21 | X | 26,181 | 12 | 0 | 초기 JSON 복구는 성공했지만 `OFF_PLAN` 뒤 재계획 응답이 비JSON이라 실패 |

v2의 ReAct 성공률은 2/3이었다. Plan-then-Execute도 1/3 성공으로 v1과 같은
성공률이었지만, 19번에서 새 복구 정책은 첫 계획의 비JSON 응답을 유효한 JSON
계획으로 전환해 최종 답변까지 도달했다. 20번은 예외가 `Meter` 값이 반환되기
전에 발생해 토큰·반복 수를 측정할 수 없었으므로, Plan-then-Execute v2의 평균
토큰·평균 반복 수는 계산하지 않았다.
...
첫 응답 `read_file(app.log)`는 v1이라면 즉시 실패했지만, 고정된 교정 프롬프트
...
이 고정 태스크와 3회 표본에서는 초기 형식 오류 복구가 한 실패 모드를 줄일 수
있음을 확인했지만, 전체 실행 안정성에는 실행 단계의 재계획과 예외 처리도 영향을
준다는 결과로 해석한다. ReAct도 v2에서 2/3 성공이므로, 두 하네스의 일반적 우열은
더 많은 반복 없이 단정하지 않는다.
